...
> source: [re2c syntax](http://re2c.org/manual/syntax/syntax.html)
...
- `@stag` 将@stag匹配的最后一个输入位置保存在名为stag的变量中
...
用于保存与匹配规则中的捕获括号相对应的s标签的值的l值的数组。数组长度必须至少为 `yynmatch * 2` (理想情况下为 `YYMAXNMATCH * 2`)。仅与-P --posix-captureures选项一起使用。
...
允许定制re2c地址标记变量的方式:默认情况下，它会发出形式为yyt的表达式，但如果标记变量被定义为结构体中的字段，或者由于任何其他原因需要特殊访问器，这可能会很不方便。例如，设置re2c:tags:expression = p - > @@将导致p-> yyt。
...
控制YYFILL后面括号中的参数。如果为零，则省略参数。如果非零，则会生成参数，除非 `re2c:define:YYFILL:naked` 被设置为非零。
